...
const getNavigatorHTML = (today) => {
  return html`
    <button class="move-month-button" onclick="previousMonthButtonDidPress()">
      ← ${MONTHS[today.getMonth() == 0 ? 11 : today.getMonth() - 1]}
      ${today.getMonth() <= 0 ? today.getFullYear() - 1 : today.getFullYear()}
    </button>
    <h1 class="month-display">
      ${MONTHS[today.getMonth()]}<br />${today.getFullYear()}
    </h1>
    <button class="move-month-button" onclick="nextMonthButtonDidPress()">
      ${MONTHS[today.getMonth() == 11 ? 0 : today.getMonth() + 1]}
